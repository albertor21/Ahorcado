...
public class Ahorcado {
	...
	Ahorcado(String word){
		this();
		this.word = word;	
		String guess = new String(new char[this.word.length()]).replace('\0', '-');	
		this.guess = guess;
	}
	...
	public void guessLetter (char letter){
		...
		if (!found) this.fails.add(letter);	
	}
	...
	public void InitializeDictionary() throws FileNotFoundException{
		dictFile = new Scanner(new File(this.getDictionaryPath()));
	}
	...
	/*devuelve null si no encuentra ninguna palabra que cumpla el criteio
	 * o sea que no la tenga en el diccionario*/
	...
	public Character getNextGuess() {
		/*Primero se devuelven las 4 primeras letras mas frecuentes del
		 * idioma seleccionado en orden aleatorio para dar idea de espontaneidad
		 * Se toman de la tercera linea del archivo del diccionario
		 */
		if (getTries()==0){
			try{
				InitializeDictionary();
			...
			String[] mostFreqArray = dictFile.nextLine().toLowerCase().split(",");
			for (int c=0; c<4 ; c++){
				mostFrequent.add(mostFreqArray[c]);
			}			
		}
		if (getTries()<4){
			int len = mostFrequent.size();
			int azar = (int) Math.floor(Math.random()*len); 
			Character result = mostFrequent.get(azar).charAt(0);
			mostFrequent.remove(azar);
			return result;
		}
		/*A partir de aqui las siguientes sugerencias se obtienen asi:			
		/*a partir de la lista de las N palabras que cumplen con el actual conjunto
		 * de Guesses (obtenidas con getListOfSolutions) se calcula la siguiente 
		 * sugerencia de guess a partir de la frecuencia de letras de esta lista y 
		 * NO de la frecuencia establecida para el idioma
		 * DEVUELVE Null si listOfSolutions is NULL*/
		...
		ArrayList<Query> listOfChars = new ArrayList<Query>(); //reutilizo Query
		for (Query q: list){
		...
		if (list.size()==2){
			...
			// O SEA: coger la letra con menor frecuencia en el calculo que se hace a continuacion 
			...
			Collections.sort(listOfChars, new Comparator<Query>() {
			    @Override
			        ...
			        {
			    		int result = 0;
			    		if (q1.getFreq()<q2.getFreq()) result =-1;
			    		if (q1.getFreq()>q2.getFreq()) result =1;
			            return result;
			        }
			    });
		...
		if (listOfChars.isEmpty()) return null;
		Collections.sort(listOfChars, new Comparator<Query>() {
	    @Override
	        public int compare(Query q1, Query q2)
	        {
	...
	private int posCharinList (ArrayList<Query> list, char character){
		for (int c=0; c< list.size(); c++){
			Query q  = list.get(c);
			if (q.getText().charAt(0)==character) return c;			
		}
		return -1;
	}
